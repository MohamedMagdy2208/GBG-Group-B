...
export function UserManagement() {
  ...
  const create = useMutation({
    mutationFn: async (payload: Record<string, FormDataEntryValue>) => (await api.post("/users", payload)).data,
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[1fr_340px]">
      ...
      <form onSubmit={submit} className="space-y-3 self-start rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <button className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Create user</button>
      </form>
    </section>
  );
}
